...
## Feature: arc-align Enhancement (Spec Scanning, Analysis Phase, cw-research Integration)

### Context
The current arc-align skill scans for scattered product-direction content (keywords + structural patterns in markdown files) and imports into BACKLOG/VISION/CUSTOMER. The user wants three enhancements:
...
3. **cw-research integration:** Should arc-align invoke cw-research as a subagent, or borrow its exploration patterns (grep through code comments, README, etc.) directly?
4. **Code scanning:** Should arc-align also grep through source code (e.g., TODO comments in `.py`, `.ts`, `.go` files), or stay limited to markdown/text files?
