...
Feature: Operational Artifact Path Restructuring
  ...
  Scenario: Align report is written to the new path
    Given a repository where /arc-align completes a full run
  ...
  Scenario: Align analysis is written to the new path
    Given a repository where /arc-align completes a full run with the analysis phase
  ...
  Scenario: Review report is written to the new path
    Given a repository where /arc-review completes a full run
  ...
  Scenario: Product-direction artifacts remain at docs/ root
    Given a repository where /arc-align completes a full run with imports
  ...
  Scenario: Hardcoded exclusion list references new operational paths
    Given the arc-align SKILL.md has been updated for path restructuring
  ...
  Scenario: All Arc skill SKILL.md files reference new paths
    ...
    When the arc-align SKILL.md is inspected
    ...
    Then all operational artifact references use docs/skill/arc/ paths
    When the arc-review SKILL.md is inspected
    Then all operational artifact references use docs/skill/arc/ paths
    When the arc-readme SKILL.md is inspected
    Then all operational artifact references use docs/skill/arc/ paths

  Scenario: Reference documents reflect new paths
    Given path restructuring has been applied to reference documents
    When skills/arc-align/references/align-report-template.md is inspected
    Then the output path references docs/skill/arc/
    When skills/arc-align/references/import-rules.md is inspected
    Then manifest path references use docs/skill/arc/
    When skills/arc-review/references/review-report-template.md is inspected
    Then the output path references docs/skill/arc/
    When skills/arc-review/references/audit-dimensions.md is inspected
  ...
  Scenario: docs/skill/arc/ directory is created during bootstrap
    ...
    When /arc-align runs and reaches the artifact bootstrap step
  ...
  Scenario: Migration does not delete old-path files automatically
    ...
    When /arc-align completes with migration
  ...
  Scenario: No migration needed when old-path artifacts do not exist
    ...
    When /arc-align runs for the first time
