...
Feature: Alignment Report
  ...
  Scenario: Report file is generated with run metadata section
    Given a completed "/arc-assess" run that scanned 50 files with 2 exclusion patterns
  ...
  Scenario: Report groups imported items by target artifact
    Given an "/arc-assess" run that imported 3 BACKLOG items, 1 VISION item, and 2 CUSTOMER items
    When the report is generated
  ...
  Scenario: Report lists excluded files and directories
    Given an "/arc-assess" run that excluded "node_modules/", "vendor/", and "data/"
    When the report is generated
  ...
  Scenario: Report shows remaining unmanaged content below confidence threshold
    Given an "/arc-assess" run that detected 2 content blocks with ambiguous product-direction signals
  ...
  Scenario: Inline summary presents correct counts as a markdown table
    Given an "/arc-assess" run that imported 4 items, skipped 2 items, and left 1 item behind
  ...
  Scenario: Next steps are offered after the report is presented
    Given a completed "/arc-assess" run with the inline summary displayed
