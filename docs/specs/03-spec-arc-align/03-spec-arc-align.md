...
- Test: Running `/arc-assess` on a repo with scattered TODOs, a README roadmap section, and a `PERSONAS.md` file produces a discovery list with correct artifact classifications
...
**Purpose:** Create the `/arc-assess` skill definition following Arc's established patterns, update plugin metadata, and integrate into the README and skills directory.

**Functional Requirements:**

- The system shall create `skills/arc-assess/SKILL.md` with the standard Arc SKILL.md frontmatter (`name`, `description`, `user-invocable: true`, `allowed-tools`) and the full process protocol
- The system shall create `skills/arc-assess/references/detection-patterns.md` documenting all keyword and structural patterns with examples
- The system shall create `skills/arc-assess/references/import-rules.md` documenting the artifact classification rules, stub generation logic, and cleanup behavior
- The system shall update `.claude-plugin/plugin.json` to include `arc-assess` in the skills list and bump the patch version
- The system shall update `README.md` to add `/arc-assess` to the skills table, pipeline diagram, and relationship section
- The system shall update `skills/README.md` (if present) to include `/arc-assess`

**Proof Artifacts:**

- File: `skills/arc-assess/SKILL.md` exists with valid frontmatter and complete process protocol
- File: `skills/arc-assess/references/detection-patterns.md` documents all scan patterns
- File: `skills/arc-assess/references/import-rules.md` documents classification and import logic
- File: `.claude-plugin/plugin.json` includes `arc-assess` and has an incremented version
- File: `README.md` references `/arc-assess` in the skills table and pipeline diagram

## Non-Goals (Out of Scope)

- **Shaping imported items** — `/arc-assess` only captures stubs; refining happens via `/arc-shape`
...
- Process steps: numbered steps with AskUserQuestion for user interactions, following arc-audit's pattern
- Reference docs: stored in `skills/arc-assess/references/` following arc-shape and arc-wave conventions
- Conventional commits: `feat(arc-assess): ...` for new files, `docs(arc-assess): ...` for documentation
- Marker comments: `<!-- aligned-from: ... -->` follows the pattern of `<!-- stale: reviewed ... -->` in arc-audit
...
- **Allowed tools:** `Glob, Grep, Read, Write, Edit, AskUserQuestion` — matches the pattern of arc-audit (no Agent needed since scanning is sequential, not parallel-dimension analysis)
...
- Running `/arc-assess` on a repo with 5+ scattered product-direction files results in all items consolidated into Arc artifacts
- Re-running `/arc-assess` after a successful run produces zero new imports (idempotent)
