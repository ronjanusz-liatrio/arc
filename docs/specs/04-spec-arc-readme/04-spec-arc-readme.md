...
4. Add WA-7 README trust-signal audit to `/arc-review` so structurally incomplete or drifted READMEs surface during health checks
5. Maintain clean namespace separation — `/arc-readme` owns `ARC:` sections only, never touches `TEMPER:` or manually-authored content
...
- As a product owner running `/arc-review`, I want to be warned when Arc-managed README sections are stale or structurally incomplete so I can run `/arc-readme` to fix it
- As a project bootstrapper, I want `/arc-readme` to scaffold a complete README from my VISION and CUSTOMER docs so I don't start from a blank file
...
**Purpose:** Add a new audit dimension to `/arc-review` that validates Arc-managed README sections against a structural trust-signal framework, completing the feedback loop that makes `/arc-readme` discoverable.
...
- The system shall add a WA-7 check to `skills/arc-review/references/audit-dimensions.md` following the exact format of WA-1 through WA-6
...
- The system shall update `skills/arc-review/SKILL.md` to include WA-7 in Step 3 (Wave Alignment Audit)
- The system shall recommend "Run `/arc-readme`" as the fix action, listing which signals failed
- The system shall skip WA-7 gracefully if README.md does not exist (report as info: "No README.md found")
- The system shall skip WA-7 gracefully if README.md contains no `ARC:` managed section markers (report as info: "No ARC: sections in README — run `/arc-readme` to scaffold")
- The system shall create `skills/arc-readme/references/trust-signals.md` defining the 8 signals with detection logic, used by both WA-7 and `/arc-readme` post-update validation

**Proof Artifacts:**

- File: `skills/arc-review/references/audit-dimensions.md` contains WA-7 section with trust-signal framework, detection logic, severity, output format, and interactive fix
- File: `skills/arc-readme/references/trust-signals.md` contains the canonical trust-signal definitions shared by WA-7 and `/arc-readme`
- File: `skills/arc-review/SKILL.md` references WA-7 in Step 3
- File: `skills/arc-review/references/review-report-template.md` includes WA-7 trust-signal scorecard in the wave alignment findings table

### Unit 2: `/arc-readme` Scaffold Mode
...
- The system shall create `skills/arc-readme/SKILL.md` with frontmatter matching Arc's skill conventions (name, description, user-invocable, allowed-tools)
...
- The system shall create `skills/arc-readme/references/readme-mapping.md` defining the artifact-to-section mapping rules
- The system shall create `skills/arc-readme/references/readme-quality-rules.md` defining quality gates adapted from readme-author (100-200 line target, accessibility basics, progressive disclosure)

**Proof Artifacts:**

- File: `skills/arc-readme/SKILL.md` exists with complete skill definition
- File: `skills/arc-readme/references/readme-mapping.md` exists with artifact-to-section mapping
- File: `skills/arc-readme/references/readme-quality-rules.md` exists with quality gates
- CLI: Running `/arc-readme` on a project with VISION.md but no README.md produces a scaffolded README with all 6 managed section markers
- CLI: Trust-signal scorecard in scaffold output shows all evaluable signals passing

### Unit 3: `/arc-readme` Update Mode
...
- CLI: Running `/arc-readme` on a project with existing README.md containing `ARC:` markers updates only managed sections
- CLI: Running `/arc-readme` shows diff summary before writing
...
**Purpose:** Wire `/arc-readme` into the Arc pipeline so it's offered after `/arc-wave`, registered in the plugin manifest, and documented in the skill hub.

**Functional Requirements:**

- The system shall update `skills/README.md` to add `/arc-readme` to the skill table and workflow diagram
- The system shall update the workflow diagram to show `/arc-readme` as a step after `/arc-wave`:
  ```
  /arc-align → /arc-capture → /arc-shape → /arc-wave → /arc-readme → /cw-spec
  ```
- The system shall update `.claude-plugin/marketplace.json` to mention `/arc-readme` in the plugin description
- The system shall update the project `README.md` (Arc's own) to list `/arc-readme` in the Skills section and the Two-Plugin Pipeline diagram
- The system shall add `/arc-readme` as a next-step option in `/arc-wave`'s Step 11 (next steps AskUserQuestion), alongside the existing `/cw-spec` option
- The system shall add "Run `/arc-readme`" as a next-step option in `/arc-review`'s Step 8 when WA-7 detects staleness
- The system shall bump the plugin version in `.claude-plugin/plugin.json` from `0.4.0` to `0.5.0`

**Proof Artifacts:**

- File: `skills/README.md` lists `/arc-readme` with description and invocation syntax
- File: `.claude-plugin/marketplace.json` description includes `/arc-readme`
- File: `.claude-plugin/plugin.json` version is `0.5.0`
- File: `skills/arc-wave/SKILL.md` offers `/arc-readme` as a next step
- File: `skills/arc-review/SKILL.md` offers `/arc-readme` when WA-7 triggers
- File: `README.md` (Arc's own) includes `/arc-readme` in skills table and pipeline diagram
...
- Automatic invocation — `/arc-readme` is always user-triggered or suggested, never auto-executed
...
- The trust-signal framework is defined once in `skills/arc-readme/references/trust-signals.md` and consumed by both WA-7 (audit) and `/arc-readme` (post-update validation) — single source of truth for detection logic
...
- References: skill-specific docs in `skills/arc-readme/references/`
- Conventional commits: `feat(arc-readme): ...`
...
- `trust-signals.md` must be structured so both WA-7 (in arc-review) and `/arc-readme` (post-update) can reference the same detection logic without duplication

## Security Considerations

- `/arc-readme` reads Arc artifacts (VISION, CUSTOMER, BACKLOG, ROADMAP) which may contain sensitive product strategy — the skill only writes to README.md and diagram files, never to external systems
...
- After running `/arc-readme`, all shipped BACKLOG items appear in the README features section
- After running `/arc-wave` followed by `/arc-readme`, the README roadmap section reflects the current wave
