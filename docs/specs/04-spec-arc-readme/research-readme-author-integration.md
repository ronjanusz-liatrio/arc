...
2. **New `/arc-readme` skill** (medium effort, high value) — Invoke readme-author patterns to generate/update README from Arc artifacts after features ship
...
5. **Cross-plugin pipeline extension** (high effort, high value) — Full end-to-end: `arc-wave → SDD → ship → arc-readme → readme-author`
...
arc/
  ...
  skills/
    arc-align/                        # Discover & import scattered content
...
**Trigger:** After features ship (post `/cw-validate` or `/temper-progress`), or on-demand.
...
**What:** Add a new audit dimension to `/arc-review` that checks if README.md is stale relative to shipped waves.
...
Recommended: Run /arc-readme or manually update features section.
```

**Interactive fix:** Offer to run `/arc-readme` or mark as acknowledged.
...
```
/arc-wave → /cw-spec → /cw-plan → /cw-dispatch → /cw-validate → /temper-progress → /arc-readme
                                                                                         ↓
                                                                                   readme-author
                                                                                   (quality rules)
```

**How:** After `/temper-progress` advances the project phase, `/arc-readme` runs automatically (or is offered as a next step) to update the README with:
...
| `/arc-readme` skill (standalone) | Medium | Medium | High |
| `/arc-readme` invoking readme-author | High | High | Very High |
| WA-7 audit check | None | Low | Medium |
| Claude plugin template contribution | None | Low | Medium |

### Recommended Sequence

1. **Start with WA-7** (low effort, immediate value from arc-review)
2. **Add managed sections** to arc-wave's existing CLAUDE.md injection step
3. **Build `/arc-readme`** as a standalone skill using readme-author's quality rules as reference docs
...
readme-author enforces several quality gates that `/arc-readme` should respect:
...
**Feature:** `/arc-readme` — Arc-aware README generation and maintenance
...
- `skills/arc-readme/SKILL.md` — New skill definition
- `skills/arc-readme/references/readme-mapping.md` — Artifact-to-README section mapping rules
- `skills/arc-readme/references/readme-quality-rules.md` — Quality gates adapted from readme-author
- `skills/arc-review/references/audit-dimensions.md` — Add WA-7 README Staleness check
...
1. WA-7 README staleness check in arc-review (audit-only, no changes)
2. `/arc-readme` scaffold mode — generate initial README from VISION + CUSTOMER artifacts
3. `/arc-readme` update mode — sync managed sections from BACKLOG shipped items + ROADMAP
4. Pipeline integration — arc-readme offered as next step after temper-progress phase advance

**Code references:**
- `skills/arc-wave/references/bootstrap-protocol.md` — Managed section injection pattern
- `skills/arc-review/references/audit-dimensions.md` — Health check pattern (for WA-7)
