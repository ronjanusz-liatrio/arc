...
Format for `docs/align-report.md`, generated by `/arc-align` after scanning the codebase for scattered product-direction content and importing discoveries into Arc-managed artifacts.
...
| Remaining unmanaged | {count of items below confidence threshold or user-rejected} |
...
Items that the scanner detected but skipped because their `{source_path}:{line_range}` key already appeared in `docs/align-manifest.md`. This section helps users understand what was intentionally not re-imported.
...
- Weak-signal matches where the keyword appeared in a non-product context (e.g., `vision` in a CSS class name)
- Ambiguous content that did not clearly map to any artifact target
