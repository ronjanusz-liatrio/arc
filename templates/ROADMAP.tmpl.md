...
- **Current Wave:** The active delivery cycle with: wave name, goal (1 sentence), 3-5 selected ideas (linked to BACKLOG sections), target timeframe (optional), and status (planned | active)
...
- Target Timeframe is optional at Vertical Slice and later phases. When provided, it can be approximate ("Q2 2026" or "next 2-3 weeks"). When no estimate is captured, render the field as `TBD (use /arc-wave to add)` in the detailed wave entry and `TBD` in the Wave Summary Table.
...
- **Wave Summary Table:** Table listing all planned waves with columns: Wave Name, Goal, Status, Target Timeframe, Idea Count. Render the Target Timeframe cell as `TBD` when no estimate is captured for a wave.
...
- **Target Timeframe (optional):** Specific timeframe (month or quarter). When no estimate is captured, render the field as `TBD (use /arc-wave to add)` in the detailed wave entry and `TBD` in the Wave Summary Table.
...
- Target Timeframe remains optional — waves may be planned without a committed estimate and backfilled later; use the documented `TBD (use /arc-wave to add)` placeholder when unset
